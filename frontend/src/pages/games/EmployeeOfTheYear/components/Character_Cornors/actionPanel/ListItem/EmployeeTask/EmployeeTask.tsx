...
import EmployeeTaskCard from "./EmployeeTaskItem";
...
const EmployeeActionList = ({ setMessage }: ActionListProp) => {
  ...
  return (
    <>
      ...
      <List
        ...
        size="small"
      >
        {status.employee_pending_tasks.map((item) => (
          <List.Item key={item.taskid} style={{ padding: "1px" }}>
            <EmployeeTaskCard
              taskid={item.taskid}
              taskTitle={item.taskTitle}
              charcater={item.charcater}
              time={item.time}
...
export default EmployeeActionList;
